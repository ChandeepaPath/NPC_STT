...
void SHTNSearch::ZoomToFitNodes(const SGraphEditor& GraphEditor, TArrayView<UEdGraphNode* const> Nodes) const
{
	...
	{
		FVector2D MinCornerOfNode(FLT_MAX, FLT_MAX);
		FVector2D MaxCornerOfNode(-FLT_MAX, -FLT_MAX);
		if (GraphPanel->GetBoundsForNode(Node, MinCornerOfNode, MaxCornerOfNode))
		{
			MinCorner = FVector2D(FMath::Min(MinCorner.X, MinCornerOfNode.X), FMath::Min(MinCorner.Y, MinCornerOfNode.Y));
			MaxCorner = FVector2D(FMath::Max(MaxCorner.X, MaxCornerOfNode.X), FMath::Max(MaxCorner.Y, MaxCornerOfNode.Y));
		}
	}
