...
struct FHTNNodeInstancePool
{
	...
	UPROPERTY()
	TArray<class UHTNNode*> Instances;
};
...
private:
	...
	UPROPERTY()
	class UHTN* PendingHTNAsset;

	UPROPERTY(Transient)
	class UHTNPlanInstance* RootPlanInstance;

	UPROPERTY(Transient)
	TMap<FHTNPlanInstanceID, class UHTNPlanInstance*> SubPlanInstances;
	...
	UPROPERTY()
	TMap<TSubclassOf<class UHTNExtension>, UHTNExtension*> Extensions;
	...
	UPROPERTY()
	TArray<class UAITask_MakeHTNPlan*> PlanningTasksPool;
	...
	UPROPERTY()
	TMap<class UHTNNode*, FHTNNodeInstancePool> NodeInstancePools;
...
FORCEINLINE const UHTNPlanInstance* UHTNComponent::FindPlanInstanceBy(PredicateType&& Predicate) const
{
	...
	for (const TPair<FHTNPlanInstanceID, UHTNPlanInstance*>& Pair : SubPlanInstances)
	{
...
{
	Invoke(Callable, *const_cast<const UHTNPlanInstance*>(RootPlanInstance));
	for (const TPair<FHTNPlanInstanceID, UHTNPlanInstance*>& Pair : SubPlanInstances)
	{
		check(IsValid(Pair.Value));
		Invoke(Callable, *const_cast<const UHTNPlanInstance*>(Pair.Value));
	}
}
...
FORCEINLINE void UHTNComponent::ForEachPlanInstance(CallableType&& Callable)
{
	...
	TArray<UHTNPlanInstance*, TInlineAllocator<16>> SubInstances;
