...
// A wrapper around an HTNPlan that contains and manages the runtime data for it, such as the plan memory and the node instances.
// It can have subplans which may be replanned during execution.
// 
// The root plan instance of an HTNComponent will keep executing in a loop forever, but instances of subplans will only do so once and are governed by their respective HTNTask_SubPlan nodes.
