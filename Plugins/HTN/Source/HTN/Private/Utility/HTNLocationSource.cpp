...
int32 FHTNLocationSource::GetLocations(TArray<FVector>& OutLocations, const AActor* Owner, const UWorldStateProxy& WorldStateProxy) const
{
	...
	if (NumAddedLocations > 0)
	{
		...
		const int32 NumAddedLocationsRemaining = Algo::StableRemoveIf(AddedLocations, [](const FVector& Location) { return !FAISystem::IsValidLocation(Location); });
		OutLocations.SetNum(OldNumLocations + NumAddedLocationsRemaining, /*bAllowShrinking=*/false);
